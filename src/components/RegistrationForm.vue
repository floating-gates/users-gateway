...
import { start_recovery_referral_passwd } from '../referral_handler/recovery.js'
...
const errors = reactive({
    ...
    email: '',
    password: '',
    postal_code: '', // Added for validation
    country: '',     // Added for validation
    logo: '',
    role: ''
})
...
<div class="untree_co-hero" id="register-section">
  <div class="container">
    <div v-if="showEmailCheck" class="email-check-container" data-aos="fade-up">
      <h3 class="heading">Check your inbox (or your Spam)!</h3>
      <p>
    ...
    <div v-else>
      <h3 class="heading" data-aos="fade-up">Publish your CAD online</h3>
      ...
      <form @submit.prevent="handleSubmit" data-aos="fade-up" data-aos-delay="100">
        ...
        <div class="form-group">
          ...
          <select v-model="form.country" class="form-control">
            ...
            <option v-for="c in countries" :key="c" :value="c.label">{{ c.name }}</option>
          </select>
        ...
        <div class="form-group">
          ...
          <div class="mb-3">
            <input id="logo-upload" type="file" accept=".png,image/png" class="hidden" @change="handleLogoUpload" />
          </div>
          <label for="logo-upload" class="upload-button">
            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" :stroke="themeColor" style="width: 32px; height: 32px;">
        ...
        <div class="button-row">
          <button type="submit" class="btn btn-primary" :style="{ background: themeColor, borderColor: themeColor }" :disabled="isLoading">
            {{ isLoading ? 'Registering...' : 'Register' }}
          </button>
          <button class="btn btn-google" type="button" @click="handleGoogleAuthentication">
            Register with
...
.hidden { display: none; }
.heading { font-size: 3rem; margin-bottom: 2rem; }
.upload-button:hover { transform: translateY(3px); cursor: pointer; }
.preview img { max-width: 120px; border-radius: 8px; border: 1px solid #ddd; }
