...
async function handleSubmit() {
    ...
    try {
        ...
        Object.entries(form).forEach(([key, value]) => {
            if (value !== null && value !== '') registrationForm.append(key, value)
        })
        ...
        const login_response = await referral_login(form.email, form.password)
        if (login_response.ok) {
            submitted.value = true
            window.location.replace("/referral-dashboard")
        } else {
